...
class Ajouter_Employe(QWidget, Ui_Add_Employee):
    ...
    def add(self):
        ...
        if (self.cin_aj_emp.hasAcceptableInput()) and (self.mat_aj_emp.hasAcceptableInput()) and ok:
            ...
            self.model.setData(self.model.index(nbr, 0), matricule_employe)
            self.model.setData(self.model.index(nbr, 1), cin_employe)
            self.model.setData(self.model.index(nbr, 2), nom_employe)
            self.model.setData(self.model.index(nbr, 3), prenom_employe)
            self.model.setData(self.model.index(nbr, 4), "P")
            self.model.setData(self.model.index(nbr, 6), date_embauche)
            self.model.setData(self.model.index(nbr, 5), date_naissance)
            # self.model.setData(self.model.index(nbr - 1, 6),str(dir + "" + photo))
            self.model.setData(self.model.index(nbr, 7), 'admin_02')
